...
head(hkc)
hkc = merge(hkc,h)
hkc$error = abs(hkc$trueAge-hkc$estAge)/hkc$height
hkc$clockModel = factor(hkc$clockModel,levels=c("trilnormcave","trilnormvex","trilnorm"),labels=c("Trimodal1","Trimodal2","Trimodal3"))
hkc$selected = "Others"
hkc[hkc$k == "selected",]$selected = "Cross-validation"
hkc[hkc$k == 50,]$selected = "Default (k=50)"
hkc$selected = factor(hkc$selected, levels=c("Default (k=50)","Cross-validation","Others"))
