...
d1r = read.table("mutation_rates.txt",header=T)
d2r = read.table("results_rate_moreBEAST.txt",header=T)
dr=rbind(d1r,d2r)
#dr$method = factor(dr$method,levels = c("wLogDate","MD-Cat","BEAST_lnorm","BEAST_strict","BEAST_rcla"))


dr$method = factor(dr$method,levels = c("BEAST_lnorm",
                             "BEAST_strict","BEAST_rcla","RelTime","wLogDate","MD-Cat"))

ggplot(dr[!dr$method %in% c("BEAST_strict"),],aes(x=method,y=log10(exp(1))*error,fill=method)) + 
...
ggplot(dr[!dr$method %in% c("BEAST_strict"),],aes(y=method,x=log10(exp(error)),fill=method)) + 
  stat_summary(position=position_dodge(width=0.9),width=.5,
               fun.data = quantiles_95,geom="crossbar",size=0.2) +
  stat_summary(position=position_dodge2(width=0.75),
               fun.data = quantiles_75,geom="crossbar",size=0.2) +
